...
本集成分支已启用 MCP 访问保护，待随 2.0 发布。MCP 只接受登记过的访问地址；未配置时先接受本机地址（含 IP 直连）。用域名连接 MCP 的用户请配置 `MCP_ALLOWED_HOSTS`，浏览器客户端另配 `MCP_ALLOWED_ORIGINS`。
...
| `MCP_ALLOWED_HOSTS` | 空 | MCP 域名登记，多个逗号分隔；与实际 Host 一致（含端口） |
