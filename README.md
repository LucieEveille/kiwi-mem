...
> 🔁 **以后怎么更新到新版本？** 见 [第六步](#第六步以后怎么跟上新版本)——一条命令装好自动更新，以后不用管。
> 如果你偏好手动更新，**一定要记得加 `--build`**：
> ```bash
> git pull
...
kiwi-mem 一直在更新。部署好之后，你有两种方式跟上最新版——**都不需要懂技术，也不会丢记忆**。

#### 方式一：让它自己更新（推荐，装一次就再也不用管）
...
装好之后，服务器每天凌晨 4 点会自己检查有没有新版本：有就自动更新，没有就什么都不做。**你以后一次都不用管了。**

想看它有没有更新过：

```bash
cat kiwi-mem/update.log
```

不想自动更新了：`crontab -e`，删掉带 `update.sh` 的那一行就行。
...
它会先告诉你有哪些更新，你确认后再动手。整个过程 1–3 分钟。
...
> ⚠️ 唯一要注意的：**不要手动改仓库里的文件**（比如 `main.py`、`docker-compose.yml`）。改了的话脚本会停下来提醒你，不会默默覆盖。配置请改 `.env` 或用管理面板。
...
A: 跑 `bash scripts/update.sh --install-cron` 装一次，以后每天自动更新，再也不用管。记忆、配置、供应商设置都不会丢，数据库表结构会自动升级，更新前还会自动备份、失败自动回滚。详见[第六步：以后怎么跟上新版本](#第六步以后怎么跟上新版本)。
